...
[![Version 2.0](https://img.shields.io/badge/Version-2.0-black?style=flat)](https://github.com/cesp99/sussurro/releases)
...
- **Hotkey Mode**: Switch between *Push to Talk* (hold to record, release to transcribe) and *Toggle* (press once to start, press again to transcribe) directly from Settings — takes effect instantly, no restart *(X11 & macOS only)*
...
**Usage:** Press the configured hotkey to record (default `Cmd+Shift+Space`). In *Push to Talk* mode (default) hold to record and release to transcribe; in *Toggle* mode press once to start and again to transcribe. Cleaned text is injected into the active application.
...
The settings window lets you switch Whisper models, download models with a live progress bar, select the transcription language, change the global hotkey, and choose the hotkey mode (Push to Talk or Toggle). All changes take effect immediately — no restart required. The live hotkey recorder shows a real-time preview as you press keys.
...
The mode can be changed at any time from **Settings → Global Hotkey → Mode**. Not applicable on Wayland.
